...
void ColladaVertices::ImportVertices(OMFConverter::ModelInfo *model)
{
	...
	while (geomIndex < geomBuffer.size()) {
		...
		ProcessVertices(p_skeleton->skeletonData, meshIndex, model);
...
void ColladaVertices::ProcessVertices(ColladaSkeleton::SkeletonTree& skeleton, uint32_t meshIndex, OMFConverter::ModelInfo *model)
{
